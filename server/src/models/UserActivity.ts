...
export interface IUserActivityModel extends Model<IUserActivity> {
    getUserActivities(userId: Types.ObjectId, page?: number, limit?: number, filters?: any): Promise<{
        activities: IUserActivity[];
        pagination: { currentPage: number; totalPages: number; totalActivities: number; hasNextPage: boolean; hasPrevPage: boolean; };
    }>;
    getActivityStats(userId: Types.ObjectId, days?: number): Promise<any[]>;
    logActivity(activityData: {
        userId: Types.ObjectId; activityType: string; description: string;
        ipAddress?: string; userAgent?: string; location?: any; deviceInfo?: any;
        status?: 'success' | 'failure' | 'warning' | 'info';
        duration?: number; resourceId?: Types.ObjectId; resourceType?: string; metadata?: any;
...
UserActivitySchema.statics.getUserActivities = async function (
    userId: any, page: number = 1, limit: number = 10, filters: any = {}
) {
    const skip = (page - 1) * limit;
    const query = { userId, ...filters };
    const activities = await this.find(query).sort({ timestamp: -1 }).skip(skip).limit(limit).populate('userId', 'name email').lean();
    const total = await this.countDocuments(query);
    return { activities, pagination: { currentPage: page, totalPages: Math.ceil(total / limit), totalActivities: total, hasNextPage: page < Math.ceil(total / limit), hasPrevPage: page > 1 } };
};

UserActivitySchema.statics.getActivityStats = async function (userId: any, days: number = 30) {
...
UserActivitySchema.statics.logActivity = async function (activityData: {
    userId: any; activityType: string; description: string; ipAddress?: string; userAgent?: string;
    location?: any; deviceInfo?: any; status?: 'success' | 'failure' | 'warning' | 'info';
    duration?: number; resourceId?: any; resourceType?: string; metadata?: any;
}) {
